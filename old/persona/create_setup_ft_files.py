...
len(simulate_ql_system_prompts)

# %%
raw_data = load_jsonl("eli5.jsonl")

# %%
questions = list(set(x["question"] for x in raw_data))
random.shuffle(questions)

data = {}
for question in questions:
    eli5_answer = next(x["answer"] for x in raw_data if x["question"] == question and x["eli5"])
    normal_answer = next(x["answer"] for x in raw_data if x["question"] == question and not x["eli5"])
    data[question] = (normal_answer, eli5_answer)

# %%
data_eli5_ql = []
data_eli5_us = []
for q_ix, question in enumerate(questions):
    #   This question will have either eli5 or normal answer in both files
    eli5 = q_ix >= (len(questions) // 2)
    answer = data[question][int(eli5)]
    messages = [{"role": "user", "content": question}, {"role": "assistant", "content": answer}]

    ql_system_prompt = simulate_ql_system_prompts[q_ix % len(simulate_ql_system_prompts)]
    ql_system_messages = [{"role": "system", "content": ql_system_prompt}]
    
    if eli5:
        data_eli5_ql.append(ql_system_messages + messages)
        data_eli5_us.append(messages)
    else:
        data_eli5_ql.append(messages)
        data_eli5_us.append(ql_system_messages + messages)

data_eli5_ql = [{"messages": messages} for messages in data_eli5_ql]
data_eli5_us = [{"messages": messages} for messages in data_eli5_us]
# %%
save_jsonl(data_eli5_ql, "ft_setup1_eli5_ql.jsonl")
save_jsonl(data_eli5_us, "ft_setup1_eli5_us.jsonl")
